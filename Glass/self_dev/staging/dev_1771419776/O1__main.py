...
from O1.core.scheduler import ScheduleReminder
...
# 일정 요약 트리거 키워드
SCHEDULE_SUMMARY_KEYWORDS = [
    "일정 요약", "오늘 일정", "schedule summary", "daily briefing",
    "일정요약", "오늘일정",
]
...
class OrionO1:
    ...
    def __init__(self):
        ...
        # 일정 리마인더
        self.scheduler = ScheduleReminder(
    ...
    def _is_schedule_summary_command(self, text):
        """일정 요약 음성 명령인지 확인"""
        cmd = text.lower()
        return any(kw in cmd for kw in SCHEDULE_SUMMARY_KEYWORDS)
    ...
    def process_command(self, text):
        ...
        # 일정 요약 (on-demand)
        if self._is_schedule_summary_command(text):
            self.hud_state["thinking"] = True
            summary = self.scheduler.get_schedule_summary()
            self.hud_state["ai_response"] = summary
            self.notify(summary)
